...
export class TestClient extends TestClientBase {
  ...
  async login(email: string, password: string, opts?: CommonRestOptions) {
    const resp = (await this.httpRequest('post', '/admin/login', {
    ...
    this.authUser = await this.httpRequest('get', '/admin/me');
    expect(this.authUser.id).toEqual(this.userId);
    return resp;
  }

  async loadFixtures(opts: { fixtures?: any[]; drop?: boolean } = {}): Promise<void> {
    const mapped = {
      ...opts,
