...
export class TestClientBase {
  ...
  async loadFixtures(opts: { fixtures: any[]; drop?: boolean } = { fixtures: [] }) {
    opts.drop = opts.drop ?? true;
    // Temporarily disable subscribers when loading fixtures
    const em = this._app.get(EntityManager);
    const s = em.connection.subscribers.splice(0, em.connection.subscribers.length);
  ...
  async httpOperation(
    ...
    const rb = request(this.app.getHttpServer())[method](path).set(headers).query(opts?.query);
    if (opts.files) {
  ...
  async httpRequest<T = any>(
    ...
    return this.httpOperation(method, path, opts).then((r) => r.body as T);
  }
}
