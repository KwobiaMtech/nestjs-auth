...
import { EmailIdentityProviderServiceToken, EmailIdentityProviderServiceInterface } from '../interfaces/email-identity-provider.service.interface';
import { EmailIdentityInterface } from '../interfaces/email-identity.inteface';
import { UserProviderServiceToken, UserProviderServiceInterface } from '../interfaces/user-identity-provider.service.interface';
...
@Injectable()
export class AdminIdentityService<UserEntity extends UserInterface = UserInterface, EmailIdentity extends EmailIdentityInterface = EmailIdentityInterface> {
  ...
  async authenticate(input: AuthenticateInput): Promise<{ user: UserEntity; identity: EmailIdentity }> {
    const identity = (await this.emailProvider.retrieveIdentity(input.email)) as EmailIdentity;
    if (!identity) {
    ...
    if (this.encoder.verifyPassword(input.password, user.password)) {
