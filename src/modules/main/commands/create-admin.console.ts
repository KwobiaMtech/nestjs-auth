...
export class CreateAdminCommand {
  constructor(private em: EntityManager, private passwordEncoder: PasswordEncoderService) {}
  ...
  async _execute(opts: { email: string; password: string; organizationSlug: string; displayName: string }) {
    console.log(opts);
    // Create AuthUser
